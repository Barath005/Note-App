...
let notes: { title: string; notes: string; }[] = JSON.parse(localStorage.getItem('notes') || '[]');
let index: number = notes.length - 1;
...
$(document).ready(function () {
  ...
  saveButton.on('click', function () {
    ...
    const textareaValue: string = ($('#summernote') as any).summernote('code');
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function displayNoteByIndex(index: number) {
    // Retrieve notes from localStorage
    const notes: { title: string; notes: string; }[] = JSON.parse(localStorage.getItem('notes') || '[]');
